    ...
    <div className="space-y-6">
      <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-5 backdrop-blur-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-base font-bold text-white">German Restrukturierungsgericht Court Schedule (StaRUG & InsO)</h2>
            <p className="text-xs text-slate-400 mt-0.5">
              Judicial plan confirmation hearings (*Planbestätigungstermine*) and creditor assemblies across German District Courts (*Amtsgerichte*).
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {sorted.map((c) => (
          <div
            key={c.id}
            onClick={() => onSelect(c)}
            className="bg-[#0d1527]/80 border border-slate-800 hover:border-cyan-500/40 rounded-xl p-4 transition-all duration-200 cursor-pointer shadow-lg hover:shadow-cyan-500/5 group flex flex-col justify-between"
          >
            <div>
              <div className="flex items-center justify-between gap-2 mb-2.5">
                <div className="flex items-center gap-2">
                  <span className="font-mono font-bold text-cyan-400 text-sm">{c.ticker}</span>
                  <span className="text-[11px] bg-slate-800 text-slate-400 px-1.5 py-0.5 rounded font-mono">
                    {c.lastPriceEur}
                  </span>
                </div>
                <span className={`text-[10px] font-semibold px-2 py-0.5 rounded border ${
                  c.mechanism.includes('StaRUG')
                    ? 'bg-cyan-500/10 text-cyan-400 border-cyan-500/20'
                    : 'bg-amber-500/10 text-amber-400 border-amber-500/20'
                }`}>
                  {c.mechanism.includes('StaRUG') ? 'StaRUG' : 'Insolvenzplan'}
                </span>
              </div>
              ...
              <div className="mt-3 space-y-2 text-xs">
                ...
                <div className="bg-slate-900/90 border border-slate-800/80 rounded-lg p-2 mt-2">
                  <div className="text-[10px] text-slate-400 font-medium">Restructuring Volume:</div>
                  <div className="text-xs font-mono font-bold text-cyan-300">{c.claimOrDealValue}</div>
                </div>
              </div>
            </div>
