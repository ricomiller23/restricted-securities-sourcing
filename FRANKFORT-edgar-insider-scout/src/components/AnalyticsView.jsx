...
export default function AnalyticsView({ signals }) {
  const tradeCount = signals.filter((s) => s.signalType.includes('MAR')).length;
  const substantialCount = signals.filter((s) => s.signalType.includes('WpHG')).length;
  const dilutionCount = signals.filter((s) => s.signalType.includes('186')).length;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-slate-900/70 border border-slate-800 rounded-xl p-4 shadow-lg">
          <div className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Directors' Dealings (Art. 19 MAR)</div>
          <div className="text-xl font-bold font-mono text-emerald-400 mt-1">{tradeCount} Signals</div>
          <div className="text-[10px] text-slate-500 mt-1">Vorstand & Aufsichtsrat trades</div>
        </div>

        <div className="bg-slate-900/70 border border-slate-800 rounded-xl p-4 shadow-lg">
          <div className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Major Holdings (§ 33 WpHG)</div>
        ...
        <div className="bg-slate-900/70 border border-slate-800 rounded-xl p-4 shadow-lg">
          <div className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Dilution Tranches (§ 186 AktG)</div>
          <div className="text-xl font-bold font-mono text-pink-400 mt-1">{dilutionCount} Placements</div>
          <div className="text-[10px] text-slate-500 mt-1">10% cash placements without rights</div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-5">
          <h3 className="text-sm font-bold text-white mb-4 flex items-center gap-2">
            ...
            German Regulatory Signal Composition
          </h3>
          <div className="space-y-3 text-xs">
            {['Art. 19 MAR (Directors\' Dealings)', '§ 33 WpHG (Major Holdings 5%+)', '§ 33 WpHG (Major Holdings 3%+)', '§ 186 AktG (Bezugsrechtsausschluss)'].map((sig) => {
