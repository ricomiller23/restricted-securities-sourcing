...
import { Radar, ExternalLink, ChevronRight, DollarSign } from 'lucide-react';

export default function DilutionRadarView({ signals, onSelect }) {
  const dilutions = signals.filter((s) => s.signalType.includes('186') || s.signalType.includes('Bezugsrechtsausschluss'));
    ...
    <div className="space-y-6">
      <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-5 backdrop-blur-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-base font-bold text-white">German Capital Measures & Dilution Radar (§ 186 AktG Bezugsrechtsausschluss)</h2>
            <p className="text-xs text-slate-400 mt-0.5">
              Tracking 10% private placements without subscription rights (*10%-Kapitalerhöhung*), convertible notes, and new share quotations on FWB.
            </p>
          </div>
        </div>
      </div>
          ...
          >
            <div>
              ...
              <div className="p-3.5 rounded-lg bg-slate-900/90 border border-slate-800 space-y-2 text-xs">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-slate-400">Shares Issued:</span>
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-mono font-bold text-white">{s.totalValueEur}</span>
                </div>
              </div>
            </div>
            ...
            <div className="mt-4 pt-3 border-t border-slate-800/80 flex items-center justify-between text-xs text-slate-400 group-hover:text-pink-300 transition-colors">
              ...
              <div className="flex items-center gap-1">
                <span>View Prospectus / Notice</span>
