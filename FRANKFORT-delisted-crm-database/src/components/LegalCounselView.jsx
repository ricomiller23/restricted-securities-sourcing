...
export default function LegalCounselView({ issuers, onOpenEmail }) {
  const counselMap = {};

  issuers.forEach((i) => {
    const counsel = i.legalCounsel;
    if (!counselMap[counsel]) {
      counselMap[counsel] = {
        name: counsel,
        issuers: [],
        insolvenzverwalter: i.insolvenzverwalter
      };
    }
    counselMap[counsel].issuers.push(i);
  });
    ...
    <div className="space-y-6">
      <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-5 backdrop-blur-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-base font-bold text-white">German Insolvency Administrators & Restructuring Counsel Directory</h2>
            <p className="text-xs text-slate-400 mt-0.5">
              *Insolvenzverwalter*, restructuring law partners, and custodian registries managing Frankfurt AG shells (*Börsenmäntel*).
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {Object.values(counselMap).map((c, idx) => (
          <div
            key={idx}
          ...
          >
            <div>
              ...
              <div className="text-xs text-slate-400 mb-3">
                Lead Administrator: <span className="text-slate-300 font-medium">{c.insolvenzverwalter}</span>
              </div>

              <div className="space-y-1.5 mb-4">
                <div className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">
                  Associated Mandates ({c.issuers.length}):
                </div>
                {c.issuers.map((iss) => (
                  <div
                    key={iss.id}
            ...
            <div className="pt-3 border-t border-slate-800 flex justify-end">
              <button
                onClick={() => onOpenEmail(c.issuers[0])}
