...
import Section656AlertsView from './components/Section656AlertsView';
import NomadsBrokersView from './components/NomadsBrokersView';
import AnalyticsView from './components/AnalyticsView';
import CompanyDrawer from './components/CompanyDrawer';
import EmailWorkbenchModal from './components/EmailWorkbenchModal';
import seedData from './data/london_filings_outreach_seed.json';
...
export default function App() {
  ...
  const filteredCompanies = useMemo(() => {
    return companies.filter((c) => {
      const matchesSearch =
        ...
        c.nomad.toLowerCase().includes(searchTerm.toLowerCase()) ||
        c.sector.toLowerCase().includes(searchTerm.toLowerCase());

      const matchesRunway =
        runwayFilter === 'all' ||
        (runwayFilter === 'critical' && c.estimatedQuartersRunway < 2.0) ||
        (runwayFilter === 's656' && (c.filingType.includes('Section 656') || c.statutoryDistressTrigger.includes('Section 656')));
  ...
  const handleExport = () => {
    ...
    link.setAttribute('download', `LONDON_Distress_Filings_${new Date().toISOString().slice(0, 10)}.csv`);
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#0b0f19] text-slate-100">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
        {/* Banner Alert for Section 656 CA 2006 */}
        <div className="bg-gradient-to-r from-indigo-950/40 via-slate-900/60 to-slate-900/40 border border-indigo-500/20 rounded-xl p-3.5 flex items-center justify-between gap-3 text-xs">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-slate-300">
              <strong className="text-indigo-400">Section 656 Serious Loss of Capital Radar:</strong> Ingesting mandatory EGM notices for public companies whose net assets fall below 50% of called-up share capital, plus microcaps with &lt; 2.0 quarters runway.
            </span>
          </div>
            ...
            <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
              <div className="flex items-center gap-1 bg-slate-900 border border-slate-800 p-0.5 rounded-lg text-xs">
                ...
                <button
                  ...
                  className={`px-2.5 py-1 rounded text-[11px] font-medium transition-colors ${
                    runwayFilter === 'critical' ? 'bg-rose-500/20 text-rose-400 font-semibold' : 'text-slate-400 hover:text-slate-200'
                  }`}
                >
                  &lt; 2.0 Qtrs Runway
                </button>
                <button
                  onClick={() => setRunwayFilter('s656')}
                  className={`px-2.5 py-1 rounded text-[11px] font-medium transition-colors ${
                    runwayFilter === 's656' ? 'bg-amber-500/20 text-amber-400 font-semibold' : 'text-slate-400 hover:text-slate-200'
                  }`}
                >
                  Section 656 Alerts
                </button>
