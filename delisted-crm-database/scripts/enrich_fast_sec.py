...
print(f"Fast multi-threaded SEC EDGAR enrichment for all {len(records)} issuers...")
...
def fetch_sec_data(norm_cik):
    if not norm_cik: return None
...
# Fetch SEC submissions in parallel
ciks = [str(r.get('cik') or '').strip().lstrip('0') for r in records]

print("Executing parallel SEC EDGAR queries...")
sec_results = {}
with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
    future_to_cik = {executor.submit(fetch_sec_data, cik): cik for cik in ciks if cik}
...
print(f"Successfully fetched SEC EDGAR submissions for {len(sec_results)} CIKs!")
...
for item in records:
    ...
    sec_info = sec_results.get(norm_cik)
    if sec_info:
        if sec_info.get('phone'):
            item['phone'] = format_phone(sec_info.get('phone'))
        
        addr = sec_info.get('addresses', {}).get('business') or sec_info.get('addresses', {}).get('mailing')
        if addr:
        ...
        if sec_info.get('name'):
            item['companyName'] = sec_info.get('name').title()
    ...
    # Apply explicit verified overrides (BCLI, SNBRQ, AWHL)
...
# Check BCLI and SNBRQ records
bcli_rec = next((r for r in enriched_records if r.get('ticker') == 'BCLI'), None)
snbrq_rec = next((r for r in enriched_records if r.get('ticker') == 'SNBRQ'), None)

print("\n--- BCLI VERIFIED RECORD ---")
print(json.dumps(bcli_rec, indent=2))

print("\n--- SNBRQ VERIFIED RECORD ---")
print(json.dumps(snbrq_rec, indent=2))

with open(seed_path, "w", encoding="utf-8") as f:
    json.dump(enriched_records, f, indent=2)

print(f"\nSuccessfully enriched all {len(enriched_records)} delisted issuers with SEC EDGAR + OTCMarkets data!")
