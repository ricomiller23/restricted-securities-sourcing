...
export default function ExportModal({ isOpen, onClose, issuers }) {
  ...
  const handleDownload = () => {
    if (format === 'csv') {
      ...
      link.setAttribute('download', `LONDON_AIM_Rule15_Shells_${new Date().toISOString().slice(0, 10)}.csv`);
      ...
      link.setAttribute('download', `LONDON_AIM_Rule15_Shells_${new Date().toISOString().slice(0, 10)}.json`);
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
      <div className="bg-[#0e1424] border border-slate-700/80 rounded-2xl max-w-md w-full p-6 shadow-2xl relative">
        ...
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            <h2 className="text-lg font-bold text-white">Export AIM Shell Database</h2>
            <p className="text-xs text-slate-400">Export {issuers.length} London shell records</p>
          </div>
        </div>
        ...
        <div className="space-y-3 mb-6">
          ...
          >
            ...
            <div>
              ...
              <div className="text-[11px] text-slate-400">For direct API feeds & database loading</div>
            </div>
          </div>
        </div>
