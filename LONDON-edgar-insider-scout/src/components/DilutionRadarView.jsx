...
import { Radar, ExternalLink, ChevronRight } from 'lucide-react';

export default function DilutionRadarView({ signals, onSelect }) {
  const dilutions = signals.filter((s) => s.signalType.includes('570') || s.signalType.includes('Dilution'));
    ...
    <div className="space-y-6">
      <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-5 backdrop-blur-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-base font-bold text-white">UK Capital Measures & Dilution Radar (Section 570 CA 2006)</h2>
            <p className="text-xs text-slate-400 mt-0.5">
              Tracking block admissions, institutional placings without statutory pre-emption rights, and convertible loan note conversions on AIM and LSE.
            </p>
          </div>
        </div>
      </div>
        ...
        {dilutions.map((s) => (
          ...
          >
            <div>
              ...
              <div className="p-3.5 rounded-lg bg-slate-900/90 border border-slate-800 space-y-2 text-xs">
                <div className="flex justify-between items-center">
                  <span className="text-slate-400">Capital Measure:</span>
                  <span className="font-semibold text-slate-200">{s.subCategory}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-slate-400">Shares Admitted:</span>
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-mono font-bold text-white">{s.totalValueGbp}</span>
                </div>
              </div>
            </div>
            ...
            <div className="mt-4 pt-3 border-t border-slate-800/80 flex items-center justify-between text-xs text-slate-400 group-hover:text-pink-300 transition-colors">
              <span>Admission Date: {s.filingDate}</span>
              <div className="flex items-center gap-1">
                <span>View Block Admission Notice</span>
